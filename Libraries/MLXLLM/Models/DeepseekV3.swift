...
class DeepseekV3Attention: Module {
    ...
    func callAsFunction(
        ...
        // MLA updates the cache before attention. Use the MLA helper so decode
        // promotes L==1 SDPA to fp32 and does not update the cache a second time.
...
class MoEGate: Module {
    ...
    func callAsFunction(_ x: MLXArray) -> (MLXArray, MLXArray) {
        ...
        var scoresForChoice = originalScores + e_score_correction_bias
        if nGroup > 1 {
            ...
            let groupsToDrop = nGroup - (topkGroup ?? 1)
            if groupsToDrop > 0 {
                var groupIdx = argPartition(topKGroup, kth: groupsToDrop - 1, axis: -2)[
                    .ellipsis, ..<groupsToDrop, 0...
                ]
                groupIdx = broadcast(
                    groupIdx,
                    to: [bsz, seqLen, groupsToDrop, (nRoutedExperts ?? 1) / nGroup])
                scoresForChoice = flattened(
                    putAlong(
                        groupScores,
                        stopGradient(groupIdx),
                        values: MLXArray(0.0, dtype: groupScores.dtype),
                        axis: -2),
                    start: -2,
                    end: -1)
            }
        ...
        let k = topK ?? 1
        let inds = argPartition(-scoresForChoice, kth: k - 1, axis: -1)[.ellipsis, ..<k]

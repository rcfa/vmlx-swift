...
| Symbol | Shape |
|---|---|
| `public struct ModelConfiguration: Sendable, Equatable` | `id`, `tokenizerSource`, `defaultPrompt`, `extraEOSTokens`, `eosTokenIds`, `toolCallFormat: ToolCallFormat?`, `reasoningParserName: String?`, `generationDefaults: GenerationConfigFile?`, `mtpStatus: MTPBundleStatus?`. |
...
| `public struct GenerationConfigFile: Codable, Equatable, Sendable` | Bundle-local `generation_config.json` defaults (`temperature`, `top_p`, `top_k`, `min_p`, `repetition_penalty`, `max_new_tokens`, `eos_token_id`, `do_sample`). |
| `public struct MTPBundleStatusSnapshot: Codable, Sendable, Equatable` | Bundle-local native-MTP status JSON, including `has_usable_native_mtp_tuning`, `can_auto_launch`, `requires_native_mtp_tuning_before_auto_launch`, `bundle_has_vision`, and nested `vmlx_mtp_tuning.json` metadata. |
| `public enum ModelRuntimeCapabilitySupport: String, Codable` | Tristate support value: `supported`, `unsupported`, `unknown`. Used so Osaurus does not conflate parser availability with a production support claim. |
| `public struct ModelRuntimeCapabilitySnapshot: Codable, Sendable, Equatable` | Single Osaurus status payload combining `ModelConfiguration`, `JangCapabilities`, `GenerationConfigFile`, and `MTPBundleStatusSnapshot`. Fields include `supports_text`, `supports_vision`, `supports_video`, `supports_audio`, `supports_tools`, `supports_reasoning`, `supports_native_mtp`, `cache_type`, parser stamps, and `generation_defaults`. The `ResolvedModelConfiguration` initializer preserves Osaurus's served model name rather than recomputing from the path. |
| `public struct ModelRuntimeCapabilityRequest: Codable, Sendable, Equatable` | Osaurus request-shape summary with requested `text`, `vision`, `video`, `audio`, `tools`, `reasoning`, and `native_mtp` lanes. It can be built from `UserInput` without retaining prompt text, paths, image bytes, or audio samples. |
| `public func ModelRuntimeCapabilitySnapshot.validate(request:unknownPolicy:) -> ModelRuntimeCapabilityValidationResult` | Fail-closed support-matrix validator for plugin/server routing. Unsupported lanes return `unsupported_modality`; unknown lanes return `unknown_modality_support` unless Osaurus explicitly passes `.allowUnknown`. Result JSON includes only lane/support/error fields and redacted log fields, not prompt content or filesystem paths. |
| `public func VMLXServerRuntimeSettings.validateRequest(_:capabilitySnapshot:unknownPolicy:) -> ModelRuntimeCapabilityValidationResult` | Request-time server-policy validator. It layers server toggles on top of the model support matrix so `multimodal.vlmMode = .force_off`, disabled video/audio lanes, or `mtp.mode = .off` return `server_modality_disabled` before the request reaches the model. |
...
| `public static func ToolCallFormat.fromCapabilityName(_: String?) -> ToolCallFormat?` | JANG-stamp → canonical enum. Accepts `qwen` / `qwen3_6` / `minimax` / `glm47` / `glm5*` / `deepseek` / `deepseek_v3*` / `deepseek_v4*` / `gpt_oss*` / `gpt-oss*` / `nemotron` / `gemma4` / `gemma-4*` / `bailing*` / `ling*` / `mistral` / `mistral4*` / `pixtral*` / `lfm2` / `kimi_k2` / `dsml` / `zaya_xml` / `hunyuan*` / `hy3*` plus every direct rawValue. DeepSeek V4 aliases route to DSML before the generic DeepSeek/GLM parser. Returns nil for unknown. |
...
| All family-specific parsers (`JSONToolCallParser`, `XMLFunctionParser`, `GemmaFunctionParser`, `GLM4ToolCallParser`, `KimiK2ToolCallParser`, `MiniMaxM2ToolCallParser`, `MistralToolCallParser`, `PythonicToolCallParser`, `Llama3ToolCallParser`, `DSMLToolCallParser`, `HunyuanToolCallParser`) | Constructed via `ToolCallFormat.createParser()`, never touched directly. ZAYA reuses `XMLFunctionParser` with `<zyphra_tool_call>` wrapper tags. |
...
| `static ReasoningParser.fromCapabilityName(_: String?) -> ReasoningParser?` | JANG-stamp → parser (or nil for `"none"`/`"mistral"`/`"gemma"`). Accepts canonical stamps plus suffixed direct capability aliases: `gemma4*` / `gemma-4*` / `gpt_oss*` / `gpt-oss*` → Harmony, `qwen3*` / `glm4_moe*` / `glm5*` / `deepseek*` / `laguna*` / `bailing*` / `ling*` / `hy3*` / `hy_v3*` / `hunyuan*` / `zaya*` → think XML, and explicit `mistral4*` → `[THINK]...[/THINK]`. Mistral3/Ministral3 model-type fallback stays `"none"` unless a bundle explicitly stamps a reasoning parser. |
...
| `public struct JangCapabilities: Sendable { reasoningParser, toolParser, thinkInTemplate, supportsTools, supportsThinking, supportsText, supportsVision, supportsVideo, supportsAudio, family, modality, cacheType }` | `JANGReasoningResolver.swift:119` reads parser fields; Osaurus can use the explicit per-lane support booleans to build multimodal request/error policy. Older bundles without those fields fall back to `modality` and snapshot `unknown` where support is not explicit. |

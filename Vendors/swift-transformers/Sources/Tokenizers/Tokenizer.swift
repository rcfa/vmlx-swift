...
public class PreTrainedTokenizer: @unchecked Sendable, Tokenizer {
    ...
    public func applyChatTemplate(
        ...
        var effectiveAdditionalContext = additionalContext ?? [:]
        if effectiveAdditionalContext["thinking"] == nil,
           let enableThinking = effectiveAdditionalContext["enable_thinking"]
        {
            effectiveAdditionalContext["thinking"] = enableThinking
        }
        if !effectiveAdditionalContext.isEmpty {
            ...
            for (key, value) in effectiveAdditionalContext {
...
public extension AutoTokenizer {
    ...
    static func from(
        ...
        return try VMLXTokenizers.AutoTokenizer.from(tokenizerConfig: tokenizerConfig, tokenizerData: tokenizerData, strict: strict)
    }

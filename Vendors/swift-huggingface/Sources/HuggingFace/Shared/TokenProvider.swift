...
///
/// ```swift
/// let client = HubClient(tokenProvider: "example-hf-token")
/// ```
///
...
///     "example-hf-token"                   // Final fallback
/// ])
/// ```
///
...
public indirect enum TokenProvider: Sendable {
    ...
    /// ```swift
    /// let client = HubClient(tokenProvider: .fixed(token: "example-hf-token"))
    ...
    ///
    /// ```swift
    /// let provider = TokenProvider.fixed(token: "example-hf-token")
    /// let token = try await provider.getToken()
    /// // Returns: "example-hf-token"
    /// ```

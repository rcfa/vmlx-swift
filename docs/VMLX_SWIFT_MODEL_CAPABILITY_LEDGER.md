...
| `JANGQ/ZAYA1-8B-JANGTQ4` | `zaya` / `ZayaModel` | `PARTIAL / STRICT-PROMPT BLOCKER` | Earlier current turnmatrix passes config/template, production defaults cache OFF/ON, BatchEngine single/chat/disk restore/concurrent/per-slot/TurboQuant B=2. Bundle defaults apply as `temp=0.600 topP=1.000 topK=0 rep=nil`; cache-on speed is about `64.7-66.3 tok/s`, peak RSS is about `5.1 GiB`, disk L2 has hits/stores, and SSM companion hits are recorded. Fresh stricter artifacts `20260518T_zaya_jangtq4_fresh_prod_cache/`, `20260518T_zaya_jangtq4_seed0_compare/`, and `20260518T_zaya_jangtq4_math_rootcause/` keep speed/cache green at about 66-68 tok/s, but reasoning-off direct math returns `2` for `2 + 2` under bundle defaults, seed 0, and explicit greedy. Top-k probes show both JANGTQ4 and MXFP4 rank token `2` above token `4` on the same rendered thinking-off prompt; no-hidden-guard story rows stay inside reasoning at 256 and 768 tokens. | Generic paged prefix hit is `N-A` by topology because ZAYA CCA is path-dependent and disk/SSM-backed. Do not promote stale blue-sky rows into full production readiness, and do not hide the direct-mode blocker with sampler/repetition/top-k/forced-close guards. |
...
| `dealign.ai/MiniMax-M2.7-JANG_K-CRACK` | `minimax_m2` / `MiniMaxModel` | `PARTIAL / PRODUCTION CHAT CACHE PASS` | Fresh `20260518T_minimax_m27_jangk_crack_turnmatrix_after_quant_diag_fix/` passes config/template, production defaults cache OFF/ON, BatchEngine single/chat, disk restore, B=2 concurrent, per-slot sampler, and TurboQuant-KV B=2. Bundle defaults apply as `temp=1.000 topP=0.950 topK=40 rep=nil`; MTP depth is `off`; production cache OFF/ON both pass 7/7 with coherent reasoning ON/OFF alternation and about `42-50 tok/s`; cache ON records `PROD_CACHE_STATS hybrid=false paged{hits=1,misses=6} disk{hits=0,misses=25,stores=21}`; strict TQ B=2 reports real compression counters and exact output isolation. Focused current cache proof `20260518T_minimax_jangk_growing_chat_after_harness_fix/` re-runs the production-shaped chat-template row: bundle defaults `temp=1.000 topP=0.950 topK=40 rep=nil`, MTP `off`, canonical history-boundary `[47]`, disk hit `47/83`, warm prompt `8.634s -> 0.192s`, and turn 2 returns `vmlx-cache-green` with `finish=stop`. The loader now treats the bundle's explicit 2/4/6/8 per-layer affine quantization as declared metadata and no longer emits the misleading `config-metadata mismatch patched in-memory` warning. | The raw token-prefix `batch_cache_hit` diagnostic is now marked N-A by the live matrix harness for MiniMax (`n-a:raw-prefix-diagnostic-not-production-chat-template`) because that raw Q/A prompt causes MiniMax to continue the pattern until max tokens. This is a harness classification, not a runtime guard: direct raw diagnostics remain available, and production cache proof comes from the MiniMax chat-template growing-cache row. No sampler, repetition, top-k, temperature, or forced-stop policy was added. |
...
- Live proof: small JANGTQ has coherent 3-turn chat and TQ disk round-trip.
  ...
  The focused current JANG_K cache artifact
  `docs/local/live-model-matrix/20260518T_minimax_jangk_growing_chat_after_harness_fix/`
  re-runs the production-shaped cache proof after the live-matrix harness
  update: bundle defaults apply, MTP is off, the canonical history-boundary is
  `[47]`, disk hits `47/83`, warm prompt time is `0.192s`, and turn 2 returns
...
- Current issues: the old raw token-prefix cache diagnostic is not a valid
  ...
  a cache miss. The live matrix now marks that raw MiniMax row as N-A and uses
  `BENCH_GROWING_CHAT_CACHE` as the production chat-cache proof. The pre-fix
  strict `BENCH_BATCH_TQ_B2` artifact showed a real
  ...
  policy was added. These are not full Osaurus promotion rows until the
  remaining non-MiniMax partial families clear the same live matrix.

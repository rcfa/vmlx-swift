...
- 2026-05-17 versioned Harmony/GLM alias follow-up:
  `docs/local/production-readiness/20260517T1347_harmony_glm_alias_normalization/`
  passes the broader no-hidden/parser sweep with 49/49 rows after normalizing
  hyphen/underscore/dot aliases. This pins `gemma-4`/`gemma_4` to Gemma4
...
- 2026-05-17 Gemma4 VL structured-cache follow-up:
  `docs/local/live-model-matrix/20260517T210417Z_gemma4_vl_chat_cache/`
  passes the live `BENCH_VL_CHAT_CACHE=1` row for
  `Gemma-4-26B-A4B-it-JANG_4M-CRACK`: image A cold output is grounded, same
  image replay hits the disk-backed coordinator at `308/308`, different image
  misses, and the text-only follow-up remains grounded with no raw reasoning or
  media marker leak. Peak footprint in the timed row is about 30.6 GB.
- 2026-05-17 Gemma4 live tool-call schema follow-up:
  `docs/local/live-model-matrix/20260517T212204Z_gemma4_batch_toolcall_real_schema/`
  passes `BENCH_BATCH_TOOLCALL=1` after the harness was tightened to pass a real
  `get_weather` schema through `UserInput.tools` and fail empty-output/no-tool
  rows. The live row loads `Gemma4`, reports `Tool format: gemma4`,
  `Reasoning stamp: harmony`, emits `toolCalls=1` with
  `get_weather({"location":"Tokyo"})`, stops normally at `genTokens=14`, leaks
  no raw markers to `.chunk`, and records peak footprint about 25.5 GB.
- 2026-05-17 post-toolcall focused recheck:
  `DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer xcrun swift test --filter MLXLMCommonFocusedTests`
  passes 187 tests in 26 Swift Testing suites plus the selected XCTest focused
  rows. This current run includes the Gemma4 tool-schema guard, the Harmony
  fragmentation/no-leak rows, Ling/Bailing and Hy3 parser/cache contracts,
...
- 2026-05-17 Gemma4 Harmony long-reasoning follow-up:
  `docs/local/live-model-matrix/20260517T2150_gemma4_harmony_long_reasoning/`
  keeps the no-leak `BENCH_HARMONY_CHECK` row and adds the explicit
  `BENCH_THINK_LOOP_PROBE` on/off pair. With `enable_thinking=true`, Gemma4
  produces 579 tokens at 77.9 tok/s with 1420 reasoning chars, 668 visible
  chars, normal EOS, and no unclosed reasoning. With `enable_thinking=false`,
  it produces 128 tokens at 69.7 tok/s with zero reasoning chars, 547 visible
...
- 2026-05-17 Osaurus PR/pin and extended-family audit update:
  ...
  kept Kimi and DSV4 excluded at that checkpoint and restates the no-hidden-sampler,
...
- 2026-05-17 Osaurus PR/pin lineage expansion and Jinja parity fix:
  ...
  passes 10/10 rows.
- 2026-05-17 post-Jinja focused common recheck:
  `docs/local/production-readiness/20260517T2210_post_jinja_common_focused/mlxlmcommon_focused.log`
  passes the broad active `MLXLMCommonFocusedTests` selection after the Jinja
  parser change: 189 Swift Testing rows in 26 suites plus the selected XCTest
  focused rows. This rechecks no-hidden sampler guards, parser aliases,
  Harmony fragmentation, Ling/Bailing/Hy3/Gemma4 cache contracts, Omni
  Parakeet/RADIO, MTP metadata/cache contracts, JANGTQ/Hadamard/matmul, server
  settings, VL shape guards, and the new Jinja binary-for-iterable rows.
- 2026-05-17 Nemotron Omni MXFP4 core proof:
  `docs/local/live-model-matrix/20260517T2215_nemotron_mxfp4_omni_recheck/nemotron_mxfp4_omni.log`
  passes `BENCH_OMNI=1` + `BENCH_OMNI_BATCH=1` 18/18 at 48 tokens on
  `/Users/eric/models/dealign.ai/Nemotron-Omni-Nano-MXFP4-CRACK`. Load is
  2.17 s; bundle defaults are `temp=0.600 topP=0.950 topK=0 minP=0.000
  rep=1.000`; direct rows run about 127-137 tok/s and BatchEngine rows about
  50-82 tok/s. This clears the MXFP4 core Omni coherency row, while repeated
  cache-on live audio remains partial until the separate output-quality gate is
  rooted and fixed.
...
- 2026-05-17 scope override: Kimi remains excluded by latest user direction,
  but DSV4 Flash and the rest of the non-Kimi local `~/models` inventory are
  back in the active Osaurus switch-readiness matrix. Fresh inventory artifact:
...
- 2026-05-17 Gemma4 multi-turn reasoning follow-up:
  ...
  BatchEngine chat transcript with thinking OFF -> ON -> OFF -> ON, prior
  assistant `reasoningContent` carried forward, bundle defaults
  (`temp=1.000 topP=0.950 topK=64 rep=nil`), and effort
  `low/medium/high/max`. The pre-fix 384-token effort row is retained in the
  same artifact and failed by length with reasoning-only output; the passing
  row uses an explicit 2048-token effort budget and closes with visible output.
- 2026-05-17 Gemma4 E2B Osaurus loop follow-up:
  `docs/local/live-model-matrix/20260517T_gemma4_e2b_osaurus_loop_report/`
  uses the local Osaurus bundle
  `/Users/eric/osaurus_models/finished/gemma-4-e2b-it-4bit` after checking that
  no duplicate `~/models` E2B bundle exists. It passes template smoke,
  `BENCH_PROD` cache OFF/ON, BatchEngine chat, TurboQuant B=2 isolation, VL
  chat/cache, VL batch chat, and the corrected reasoning turn matrix. The
  retained 256-token thinking-on row failed by length before visible output;
  the 1536/2048-token re-run closes with visible output, no raw marker leak,
...
| MTP activation must use real tensor evidence and bundle tuning, not names. | `9b1b254`; `docs/VMLX_SWIFT_MTP_OSAURUS_WIRING_2026_05_15.md`; local fail-closed artifact `docs/local/native-mtp-qwen36-20260515/jang4m-crack-native-mtp-denied-postrevert.log`. Fresh settings proof and real-bundle proof under `docs/local/production-readiness/20260517T_real_mtp_auto_launch_policy/` plus the 2026-05-18 tuning-file update prove local Qwen3.6 MTP bundles require real tensor/index evidence and usable bundle-local `vmlx_mtp_tuning.json`; depth comes from that file. `Qwen3.6-27B-MXFP4-CRACK` remains `mtp=no`, missing tuning remains off/blocked, and profile/tuning-blocked force-on launch still reports a real settings error before runtime. | live-proven |
| MTP depth-3 production acceleration. | Current local Swift rows prove recursive D3 draft, accepted-prefix commit, coherent visible output, and no loop/leak for Qwen3.6 27B JANG_4M, 27B MXFP4, 27B MXFP8, 35B MXFP4, and 35B MXFP8. The fresh MXFP production reverify passes 27B/35B MXFP4/MXFP8 7/7 with `BENCH_MAX_TOKENS=384`, explicit `chunk_commit`, bundle defaults, disk L2 hit, and SSM companion hit. The latest focused artifact `docs/local/production-readiness/20260517T174743Z_qwen_mtp_chunk_policy_finalize/MTPRuntimeFocusedTests_after_prefix_snapshot_materialize.log` passes 42/42 and pins tensor-gated activation, D3 hidden-state draft/verify, accepted-prefix SSM offsets, partial-reject repair, verifier env override ordering, materialized recurrent prefix snapshots, BatchEngine exclusive native-MTP lane, quant shape-walk, and norm-convention behavior. Native MTP now auto-resolves only for supported tensor-proven Qwen bundles with usable `vmlx_mtp_tuning.json`; speed target remains partial. | live-proven for Qwen3.6 correctness rows; speed partial |
...
| VL media salt, same-image hit, changed-image miss. | `docs/local/live-model-matrix/20260516Tzaya-vl-think-template-fix/ZAYA1-VL-8B-JANGTQ4_vl_chat_cache.out`: same-media replay HIT, different-media MISS, coherent blue/orange follow-up. `docs/local/live-model-matrix/20260516Tguard-removal/Qwen3.6-27B-JANG_4M-CRACK_vl_chat_cache_final.out` and `.../Qwen3.6-27B-MXFP4-CRACK_vl_chat_cache_probe.out`: Qwen3VLProcessor same-media disk HIT `84/84`, different-media MISS, coherent follow-up. `docs/local/live-model-matrix/20260517T210417Z_gemma4_vl_chat_cache/gemma4_vl_chat_cache.out`: Gemma4Processor same-media disk HIT `308/308`, different-media MISS, and text-only follow-up remains grounded in the image colors. | live-proven for ZAYA1-VL JANGTQ4, Qwen3.6 JANG_4M/MXFP4 CRACK, and Gemma4 |
| Nemotron Omni audio/Parakeet/RADIO. | Video generation now carries the processor's post-EVS keep count and applies real EVS before prompt splice. `docs/local/live-model-matrix/20260516Tomni-nonmtp/Nemotron-Omni-Nano-JANGTQ4-CRACK_omni_evs_v2.out` passes 13/13 TokenIterator rows; strict pre-fix artifact `..._omni_strict.out` failed the video row. The second fix canonicalizes the closed no-thinking media tail to `<think>\n</think>\n\n`; tail probe `..._omni_tail_probe.out` proves compact tail fails and spaced tail grounds the same image, and `..._omni_batch_nothink_tail_fix.out` passes 18/18 including direct and BatchEngine image with `enable_thinking=false`. Fresh wrapper fix: `docs/local/live-model-matrix/20260517T170614Z_omni_live_voice_fresh_recheck/processor_audio_wrapper_red.log` proves the stale Swift audio wrapper tokenized literal `<sound>` text and missed source wrapper IDs `28`/`29`; `processor_audio_wrapper_green.log` passes after switching to `<so_start>`/`<so_end>`, and `NemotronHOmniPreEncodedAudioTests_after_audio_wrapper_fix.log` passes 9/9. Live after-fix evidence: `omni_audio_latency_jangtq4_both_paths_cache_off_32_after_audio_wrapper_fix.jsonl` reloads JANGTQ4, uses bundle defaults, pre-encodes Parakeet to `63 x 2688` in 46.7 ms, and streams raw PCM plus pre-encoded audio through BatchEngine and TokenIterator at 65.4-76.1 tok/s with no literal sound-marker leak; `omni_audio_latency_jangtq4_both_paths_cache_off_32_repeats3_after_audio_wrapper_fix.jsonl` repeats 12 cache-off rows with no marker leak and 70.6-76.6 tok/s on iterator rows; `omni_runbench_jangtq4_48_after_audio_wrapper_fix.log` passes integrated `BENCH_OMNI=1` + `BENCH_OMNI_BATCH=1` 18/18 at `maxTokens=48`. Fresh JANGTQ sibling core proof `docs/local/live-model-matrix/20260517T214045Z_nemotron_jangtq_omni_recheck/omni_jangtq_48.log` passes 18/18 at 48 tokens with bundle defaults, text/image/video/audio/direct/BatchEngine rows, media-salt isolation, and hybrid SSM warm-pass parity. Fresh MXFP4 sibling core proof `docs/local/live-model-matrix/20260517T2215_nemotron_mxfp4_omni_recheck/nemotron_mxfp4_omni.log` passes 18/18 at 48 tokens with bundle defaults, text/image/video/audio/direct/BatchEngine rows, media-salt isolation, and hybrid SSM warm-pass parity. Caveat: independently encoded Parakeet chunks remain not concat-safe, and cache-on repeated live audio remains partial until the cache-hit output-quality/root-cause gate is fixed. | live-proven for Omni JANGTQ, JANGTQ4, and MXFP4 core; cache-on repeated audio partial |
| Reasoning on/off/effort matrix. | Focused DSV4 pass-through exists. MiniMax Small now has live thinking ON/OFF alternation with `.reasoning` deltas ON and zero reasoning OFF. Ling/Bailing template has no active thinking rail in this bundle and returns visible content with no marker leak; active focused tests now also pin Bailing/Ling `enable_thinking` directive insertion/replacement. Gemma 4/GPT-OSS Harmony parser contracts now have active focused proof for Gemma 4 `<|channel>...<channel|>` and GPT-OSS `<|channel|>analysis/final<|message|>...` splitting, including `gpt_oss* -> harmony` model-type stamping. Fresh prompt-tail proof `docs/local/production-readiness/20260517T1315_harmony_prompt_tail_parser/HarmonyParserFocusedTests.log` passes 6/6 and verifies the production `ReasoningParser.forPrompt(...)` path strips GPT-OSS/Gemma4 Harmony control markers before `.chunk` emission. The fresh fragmentation/leak proof `docs/local/production-readiness/20260517T1435_harmony_fragment_leak/` passes the Harmony suite 8/8 and broader no-hidden sweep 47/47 after adding one-character chunking and stray control-token scrub coverage. The broader no-hidden-reasoning sweep `docs/local/production-readiness/20260517T1320_no_hidden_reasoning_regression/NoHiddenReasoningCloseBiasFocusedTests.log` passes 42/42 across Harmony, Hy3, Ling/Bailing, Laguna, Mistral/Ministral, Gemma4 VLM source contracts, Mistral3 dispatch, and no forced close-bias checks. The alias-normalization sweep `docs/local/production-readiness/20260517T1347_harmony_glm_alias_normalization/NoHiddenReasoningCloseBiasFocusedTests.log` passes 49/49 and adds hyphen/underscore/dot coverage for `gemma-4`, `glm-5.1-flash`, and `gpt-oss` aliases. Direct capability aliases with suffixes now also resolve instead of bypassing the parser: `gemma4_27b` / `gpt_oss_20b` / `gpt_oss_120b` route to Harmony, `glm4_moe_lite` / `glm5_air` / `deepseek_v4_flash` / `laguna_glm_thinking_v5` route to think-XML, direct Qwen-VL capability stamps `qwen3_vl` / `qwen3_5_vl` / `qwen3_6_vl` route to Qwen thinking, and explicit `mistral4*` capability stamps route `[THINK]...[/THINK]` while the `mistral4` model-type fallback stays no-reasoning unless the bundle stamps that parser. Active Hy3 focused tests pin Hunyuan/think_xml aliases plus open/closed prompt-tail reasoning separation, and now also prove preserved nextn layers are not inserted into the base decode cache. Active Laguna tests pin thinking-on/off prompt-tail routing and mixed-shape RoPE decode. Active Mistral3/Ministral3 tests pin no-reasoning fallback so literal `<think>` text is not hidden by a fake parser. Fresh Gemma4 live long-budget single-turn proof emits hidden Harmony reasoning with `enable_thinking=true` and zero reasoning with `enable_thinking=false`; fresh `BENCH_REASONING_TURN_MATRIX=1` proof adds one loaded BatchEngine growing transcript with prior assistant `reasoningContent`, OFF -> ON -> OFF -> ON routing, no visible control-token leak, no OFF reasoning, and effort `low/medium/high/max` all stopping normally with visible output when given a 2048-token effort budget. The broader model-family reasoning-effort matrix is still incomplete outside these proven families. | partial |
...
- ZAYA text JANGTQ4 and MXFP4 config/template/prod defaults. JANGTQ_K evidence
  in this snapshot is historical until a current all-non-Kimi re-run.
...
7. Expand the active model matrix beyond the 20GB cutoff for DSV4, MiniMax,
   Ling, Hy3, Gemma 4, ZAYA/ZAYA-VL, Qwen, and Nemotron one family at a time,
   with process checks before and after each run. Kimi remains intentionally
   excluded in the current sweep.

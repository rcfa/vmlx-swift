...
struct DeepseekV4ChatTemplateFallbackFocusedTests {
    ...
    func lfm2FallbackRepeatsExactRequiredToolValueAfterHistory() throws {
        ...
        #expect(afterFinalUser.contains("Output exactly the native bracketed tool call above."))
        #expect(afterFinalUser.contains("Preserve the `text` value byte-for-byte, including newlines and spacing."))
        #expect(afterFinalUser.contains("Do not append a trailing newline or any other character after the copied value."))
        #expect(afterFinalUser.contains("Do not invent placeholders, summaries, ellipsis, or prior-turn text."))
        #expect(!afterFinalUser.contains(#"<|tool_call_start|>[line_count(text="red\ngreen\nblue")]<|tool_call_end|>"#))

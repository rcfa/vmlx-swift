...
struct DeepseekV4ChatTemplateFallbackFocusedTests {
    ...
    func lfm2FallbackHonorsRequiredNamedToolChoice() throws {
        ...
        #expect(rendered.contains(#"the exact `text` value encoded with JSON \n escapes is: red\ngreen\nblue"#))
    ...
    func lfm2FallbackRepeatsExactRequiredToolValueAfterHistory() throws {
        ...
        #expect(afterFinalUser.contains(#"<|tool_call_start|>{"name":"line_count","arguments":{"text":"one\ntwo"}}<|tool_call_end|>"#))
        ...
        #expect(afterFinalUser.contains(#"In the JSON call, each line break is represented by the two characters \n"#))
        #expect(afterFinalUser.contains(#"the exact `text` value encoded with JSON \n escapes is: one\ntwo"#))
        ...
        #expect(!beforeFinalUser.contains(#"<|tool_call_start|>{"name":"line_count","arguments":{"text":"red\ngreen\nblue"}}<|tool_call_end|>"#))

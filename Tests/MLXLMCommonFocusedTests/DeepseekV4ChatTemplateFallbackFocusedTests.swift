...
struct DeepseekV4ChatTemplateFallbackFocusedTests {
    ...
    func lfm2FallbackHonorsRequiredNamedToolChoice() throws {
        ...
        #expect(rendered.contains("Required assistant message for this current request:"))
        ...
        #expect(rendered.contains("No prose, no markdown, no JSON object, no reasoning text"))
        #expect(rendered.contains("Output exactly the native bracketed tool call above."))
        #expect(rendered.contains("Preserve the `text` value byte-for-byte, including newlines and spacing."))
        #expect(rendered.contains("Do not append a trailing newline or any other character after the copied value."))
        #expect(rendered.contains("Do not output an empty `line_count()`"))
    ...
    func lfm2FallbackRepeatsExactRequiredToolValueAfterHistory() throws {
        ...
        #expect(beforeFinalUser.contains("The active API tool_choice is required"))
        #expect(beforeFinalUser.contains(#"<|tool_call_start|>[line_count(text="one\ntwo")]<|tool_call_end|>"#))
        #expect(beforeFinalUser.contains("Output exactly the native bracketed tool call above."))
        #expect(beforeFinalUser.contains("Preserve the `text` value byte-for-byte, including newlines and spacing."))
        #expect(beforeFinalUser.contains("Do not append a trailing newline or any other character after the copied value."))
    ...
    func lfm2TemplateShimOnlyEngagesStampedToolBundles() throws {
        ...
        #expect(rewritten.contains("Required assistant message for this current request"))

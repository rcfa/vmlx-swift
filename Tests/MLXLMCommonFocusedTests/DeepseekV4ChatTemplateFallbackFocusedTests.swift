...
struct DeepseekV4ChatTemplateFallbackFocusedTests {
    ...
    func lfm2FallbackHonorsRequiredNamedToolChoice() throws {
        ...
        #expect(!rendered.contains("\"name\":\"line_count\""))
        #expect(!rendered.contains("<tools>"))
        #expect(rendered.contains("Do not write reasoning, XML-style tool tags, JSON, markdown, or prose."))
        #expect(rendered.contains("Copy the `text` value exactly from the current user request, including newlines and spacing."))
        #expect(rendered.contains("Do not add a trailing newline after the copied value."))
    ...
    func lfm2FallbackRepeatsExactRequiredToolValueAfterHistory() throws {
        ...
        #expect(beforeFinalUser.contains("Copy the `text` value exactly from the current user request, including newlines and spacing."))
        #expect(beforeFinalUser.contains("Do not add a trailing newline after the copied value."))
        ...
        #expect(!beforeFinalUser.contains("\"name\":\"line_count\""))

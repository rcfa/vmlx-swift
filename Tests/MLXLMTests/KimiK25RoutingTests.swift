...
final class KimiK25RoutingTests: XCTestCase {
    ...
    func testKimiNGroupOneRouterUsesBiasOnlyForSelection() throws {
        let config = try decodeDeepseekV3Config("""
            {
              "model_type": "kimi_k25",
              "hidden_size": 2,
              "num_hidden_layers": 2,
            ...
            """)
        let gate = MoEGate(config: config)
        gate.weight = MLXArray([
            Float(0.1), 0.0,
            Float(0.2), 0.0,
            Float(0.3), 0.0,
            Float(0.4), 0.0,
        ], [4, 2])
        gate.e_score_correction_bias = MLXArray([Float(10.0), 0.0, 0.0, 0.0], [4])

        let x = MLXArray([Float(1.0), 0.0], [1, 1, 2])
        let (indices, scores) = gate(x)
        MLX.eval(indices, scores)

        let selected = indices.asArray(Int32.self).map(Int.init)
        let weights = scores.asArray(Float.self)
        XCTAssertEqual(Set(selected), Set([0, 3]))

        let raw0 = sigmoid(0.1)
        let raw3 = sigmoid(0.4)
        let expected = raw0 / (raw0 + raw3)
        guard let slot = selected.firstIndex(of: 0) else {
            return XCTFail("biased router should select expert 0")
        }
        XCTAssertEqual(weights[slot], expected, accuracy: 1e-5)
        XCTAssertLessThan(weights[slot], 0.5, "selection bias must not become expert weight")
    }
    ...
    private func decodeDeepseekV3Config(_ json: String) throws -> DeepseekV3Configuration {
        try JSONDecoder().decode(DeepseekV3Configuration.self, from: Data(json.utf8))
    }
